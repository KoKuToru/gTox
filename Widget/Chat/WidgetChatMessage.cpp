...
bool WidgetChatMessage::on_draw(const Cairo::RefPtr<Cairo::Context>& cr) {
    ...
    stylecontext->render_background(cr, 0, 0, get_allocated_width(), get_allocated_height());
    ...
    //draw the text
    m_text->set_width(Pango::SCALE * (get_allocated_width() - (padding.get_left() + padding.get_right())));
...
void WidgetChatMessage::get_preferred_height_for_width_vfunc (int width, int& minimum_height, int& natural_height) const {
    ...
    m_text->set_width(Pango::SCALE * (width - (padding.get_left() + padding.get_right())));
