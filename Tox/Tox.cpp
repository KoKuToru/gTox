...
void Tox::init(const Glib::ustring& statefile) {
  ...
  if (statefile != "") {
    ...
    try {
      ...
      int version =
          m_db->execAndGet("SELECT value FROM config WHERE name='version'")
              .getInt();
...
void Tox::save(const Glib::ustring& statefile) {
  ...
  try {
    ...
    auto runid =
        m_db->execAndGet("SELECT value FROM config WHERE name='runid' LIMIT 1")
            .getInt();
...
Tox::ReceiptNr Tox::send_message(Tox::FriendNr nr,
  ...
  if (m_db) {
    SQLite::Statement storeq(*m_db,
                             "INSERT INTO log(friendaddr, sendtime, type, "
                             "message, receipt) VALUES (?1, CURRENT_TIMESTAMP, "
                             "?2, ?3, ?4)");
...
void Tox::set_status_message(Glib::ustring msg) {
  ...
  if (tox_set_status_message(m_tox,
...
void Tox::inject_event(SEvent ev) {
  ...
  if (m_db) {
    if (ev.event == READRECEIPT) {
      ...
      SQLite::Statement storeq(*m_db,
                               "INSERT INTO log(friendaddr, recvtime, type, "
...
std::vector<Tox::SLog> Tox::get_log(Tox::FriendNr nr, int offset, int limit) {
  ...
  if (m_db) {
    SQLite::Statement loadq(*m_db,
                            "SELECT strftime('%s', sendtime), strftime('%s', "
                            "recvtime), type, message FROM log WHERE "
                            "friendaddr = ?1 ORDER BY id DESC LIMIT ?2, ?3");
