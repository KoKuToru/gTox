...
bool chat_message::is_shape(PangoLayoutRun* run) {
    if (!run) {
        return false;
    }
...
bool chat_message::on_draw(const Cairo::RefPtr<Cairo::Context>& cr) {
...
void chat_message::set_text(const Glib::ustring& text) {
...
void chat_message::force_redraw() {
...
Gtk::SizeRequestMode chat_message::get_request_mode_vfunc() const {
    return Gtk::SIZE_REQUEST_HEIGHT_FOR_WIDTH;
}

void chat_message::get_preferred_width_vfunc(int& minimum_width,
...
void chat_message::get_preferred_height_for_width_vfunc(
    int width,
    int& minimum_height,
...
void chat_message::get_preferred_height_vfunc(int& minimum_height,
                                                 int& natural_height) const {
    Glib::RefPtr<Gtk::StyleContext> stylecontext = get_style_context();
    auto padding = stylecontext->get_padding();
...
Glib::ustring chat_message::get_text() {
    return m_text->get_text();
}

void chat_message::on_selection(int from_x, int from_y, int to_x, int to_y) {
...
Glib::ustring chat_message::get_selection() {
    if (!m_clip) {
        return Glib::ustring();
    }
